...
int main(int argc, char *argv[]) {
    ...
    QWidget window;
    window.setWindowTitle("Stock Analysis");

    // 创建布局管理器
    QVBoxLayout *layout = new QVBoxLayout(&window);

    QPushButton *sortButton = new QPushButton("外排序并创建索引", &window);
    layout->addWidget(sortButton);
    QObject::connect(sortButton, &QPushButton::clicked, []() {
        string file_input_stock = "input1000.csv";
        int run_size_stock = 200000;

        string file_name_final = multi_round_2_way_merge_sort(file_input_stock, run_size_stock);

        ofstream outputFile("output.txt");
        if (!outputFile.is_open()) {
            QMessageBox::critical(nullptr, "Error", "Unable to open output.txt");
            return;
        }
        ...
        if (sortedFile.is_open()) {
            ...
            sortedFile.close();
            QMessageBox::information(nullptr, "Success", "Sorting result has been written to output.txt");
        } else {
            QMessageBox::critical(nullptr, "Error", ("Unable to open sorted file " + file_name_final).c_str());
            return;
        }
    ...
    // 创建其他按钮并添加到布局中
    QPushButton *klineButton = new QPushButton("K线图和预测", &window);
    layout->addWidget(klineButton);
    QObject::connect(klineButton, &QPushButton::clicked, []() {
        Hatu *w = new Hatu(); // 使用动态分配创建 Hatu 对象
        w->show(); // 调用 show() 方法显示窗口
    });

    QPushButton *heatmapButton = new QPushButton("热力图", &window);
    layout->addWidget(heatmapButton);
    QObject::connect(heatmapButton, &QPushButton::clicked, []() {
        rlt *w1 = new rlt(); // 使用动态分配创建 rlt 对象
        w1->show(); // 调用 show() 方法显示窗口
    });
